# Actual file is stored in /home/enoch/.oh-my-zsh/custom/themes/powerlevel10k/internal/
...
################################################################
# Segment to display the detected ISP
# p10k segment -s WARM -f yellow -i '' -t "CORE TEMP: 80"
...
prompt_my_isp_status() {
    ...
    do
       ispAndCty+=("$val")
    done <<(curl -s http://ipwhois.app/json/ | jq -r '.isp,.country_code|@sh')

    if [ "Hyperoptic Ltd" = "${ispAndCty[1]}" ] 
    then
      local state=UNLOCK
      local icon=''
      local fgc='red'
    ...
    p10k segment -f ${fgc} -i ${icon} -t " CTY:${ispAndCty[2]}"
}
